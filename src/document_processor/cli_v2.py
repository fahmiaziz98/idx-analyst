...
from src.core.exception import DocumentProcessorError, ValidationError
...
Environment Variables:
  ...
  GROQ_API_KEY          Alternative to OPENAI_API_KEY
    ...
    required.add_argument(
        ...
        type=int,
        required=True,
        help="Reporting year (e.g., 2023, 2024)"
    )
...
def validate_arguments(args):
    ...
    if args.start_page and args.end_page:
        if args.start_page > args.end_page:
            errors.append(
                f"start-page ({args.start_page}) must be <= end-page ({args.end_page})"
            )
    ...
    if args.chunk_overlap >= args.chunk_size:
        errors.append(f"chunk-overlap ({args.chunk_overlap}) must be < chunk-size ({args.chunk_size})")
